...
import net.mcreator.firstmod.entity.renderer.VoidrunnerRenderer;
import net.mcreator.firstmod.VanillaAdditionsByTrappModElements;

@VanillaAdditionsByTrappModElements.ModElement.Tag
public class VoidrunnerEntity extends VanillaAdditionsByTrappModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).immuneToFire()
			.size(1.1f, 2.9f)).build("voidrunner").setRegistryName("voidrunner");
	public VoidrunnerEntity(VanillaAdditionsByTrappModElements instance) {
		super(instance, 372);
		FMLJavaModLoadingContext.get().getModEventBus().register(new VoidrunnerRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -16777216, -16777216, new Item.Properties().group(ItemGroup.MISC))
				.setRegistryName("voidrunner_spawn_egg"));
	}

	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("vanilla_additions_by_trapp:abyssalcaverns").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
